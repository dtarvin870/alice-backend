...
import { Package, Upload, Check, AlertCircle, Trash2, ArrowLeft, Barcode, MapPin, Tablet, Edit3, Plus, Search, Loader2, Scan } from 'lucide-react';
...
const InventoryManager = () => {
    ...
    const handleDelete = async (id) => {
        ...
        try {
            const resp = await fetch(`${API_BASE}/inventory/${id}`, {
                method: 'DELETE'
            });
                    ...
                    <div className="space-y-6">
                        <div className="bg-white border border-slate-200 p-6 rounded-sm shadow-sm space-y-4">
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="relative">
                                    <Scan className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-300" />
                                    <input
                                        type="text"
                                        name="uid"
                                        value={formData.uid}
                                        onChange={handleChange}
                                        placeholder="UNIQUE UNIT ID (NFC/RFID)"
                                        className="w-full bg-slate-50 border border-slate-200 rounded-sm py-3 pl-11 pr-4 text-umbrella-black focus:outline-none focus:ring-1 focus:ring-umbrella-red transition-all font-mono uppercase tracking-widest text-xs"
                                    />
                                </div>
                            </div>
                            ...
                            {filteredInventory.map(item => (
                                <div key={item.id} className="group bg-white border border-slate-200 p-4 rounded-sm flex gap-4 hover:border-umbrella-red/50 hover:shadow-md transition-all">
                                    <div className="w-20 h-20 bg-slate-100 rounded-sm overflow-hidden flex-shrink-0 border border-slate-100 p-1">
                                        {item.photo_url ? (
                                            <img src={item.photo_url.startsWith('http') ? item.photo_url : `${BASE_URL}${item.photo_url}`} alt={item.name} className="w-full h-full object-cover" />
                                        ) : (
                                    ...
                                    <div className="flex-grow min-w-0">
                                        ...
                                        <div className="mt-2 flex flex-wrap gap-2">
                                            ...
                                            {item.uid && (
                                                <div className="text-[12px] font-mono bg-slate-800 text-white px-1.5 py-0.5 rounded-sm font-black uppercase tracking-tighter shadow-sm">
                                                    UID: {item.uid}
                                                </div>
                                            )}
                                        </div>
