...
import { ShoppingBag, Search, Plus, Minus, Trash2, CheckCircle2, User, Package, ShieldCheck, ShoppingCart } from 'lucide-react';
import { API_BASE, BASE_URL } from '../apiConfig';
            ...
            <div className="lg:col-span-2 space-y-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {filteredInventory.map(item => (
                        <div key={item.id} className="bg-white border-2 border-slate-200 p-5 rounded-sm shadow-sm hover:border-umbrella-red transition-all group">
                            <div className="flex justify-between items-start mb-4">
                                <div className="w-16 h-16 bg-slate-50 rounded-sm border border-slate-100 overflow-hidden flex items-center justify-center p-1">
                                    {item.photo_url ? (
                                        <img src={item.photo_url.startsWith('http') ? item.photo_url : `${BASE_URL}${item.photo_url}`} alt={item.name} className="w-full h-full object-cover rounded-sm" />
                                    ) : (
                                        <Package className="w-8 h-8 text-slate-200 group-hover:text-umbrella-red transition-colors" />
                                    )}
                                </div>
                                <div className="text-[12px] font-mono font-black text-black uppercase tracking-widest bg-slate-100 border border-slate-200 px-2 py-1 rounded-sm">
                                    ID: #{item.id.toString().padStart(4, '0')}
                                </div>
                            </div>
                            <h3 className="text-lg font-black uppercase tracking-tight mb-1 text-black">{item.name}</h3>
                            <p className="text-[11px] font-bold text-umbrella-red uppercase tracking-wider mb-2">{item.dosage}</p>

                            {item.upi && (
                                <div className="text-[12px] font-mono bg-slate-100 text-black px-2 py-0.5 rounded-sm font-black uppercase tracking-tighter border border-slate-200 w-fit mb-4">
                                    UPI: {item.upi}
                                </div>
                            )}
                            ...
                            <div className="flex items-center justify-between pt-4 border-t border-slate-100">
                                <div className="text-[12px] font-mono text-black">
                                    STOCK: <span className={item.stock < 10 ? 'text-umbrella-red font-black' : 'text-black font-black'}>{item.stock} UNITS</span>
                                </div>
                                <button
                            ...
                            {cart.length === 0 ? (
                                ...
                                <div key={item.id} className="flex items-center gap-3 bg-slate-50 p-3 rounded-sm border border-slate-100 animate-in slide-in-from-right">
                                    ...
                                    <div className="flex items-center gap-2 text-umbrella-black">
                                        ...
                                        <span className="text-[12px] font-mono font-black w-4 text-center">{item.quantity}</span>
                                        <button type="button" onClick={() => updateQuantity(item.id, 1)} className="p-1 hover:text-umbrella-red transition-colors">
